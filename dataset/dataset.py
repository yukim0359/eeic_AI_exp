...
class MYalignCollate_real(alignCollate_syn):
    def __call__(self, batch):
        images_HR_path, images_lr_path, label_strs = zip(*batch)
        images_HR = Image.open(images_HR_path)
        images_lr = Image.open(images_lr_path)
        imgH = self.imgH
        imgW = self.imgW
        transform = resizeNormalize((imgW, imgH), self.mask)
        transform2 = resizeNormalize((imgW // self.down_sample_scale, imgH // self.down_sample_scale), self.mask)
