...
class TextSR(base.TextBase):
    def train(self):
        ...
        model, image_crit = model_dict['model'], model_dict['crit']  # TSRNモデルと損失関数
    ...
    def test(self):
        ...
        if os.path.exists('checkpoint/{}/images'.format(self.args.exp_name)):
            shutil.rmtree('checkpoint/{}/images'.format(self.args.exp_name))
        os.mkdir('checkpoint/{}/images'.format(self.args.exp_name))

        text_label_file = open('checkpoint/{}/images/label.txt'.format(self.args.exp_name), 'w+')
        ...
        for test_dir in items:
            ...
            for i, data in (enumerate(test_loader)):
                ...
                if self.args.rec == 'moran':
                    ...
                    # store images
                    for index in range(val_batch_size):
                        lr_pil = to_pil(images_lr[index].cpu())
                        sr_pil = to_pil(images_sr[index].cpu())
                        hr_pil = to_pil(images_hr[index].cpu())
                        lr_pil.save('checkpoint/{}/images/{}_{}_lr.jpg'.format(self.args.exp_name, test_dir,
                                                                               index + sum_images))
                        sr_pil.save('checkpoint/{}/images/{}_{}_sr.jpg'.format(self.args.exp_name, test_dir,
                                                                               index + sum_images))
        ...
        text_label_file.close()

    def to_gray_tensor(self, tensor):
        R = tensor[:, 0:1, :, :]
        G = tensor[:, 1:2, :, :]
        B = tensor[:, 2:3, :, :]
    ...
    def get_mask(self, image_tensor):
        ...
        pred_mask = output[0][1]
        pred_mask_pil = to_pil((pred_mask > 0.5).type_as(image_tensor).unsqueeze(0).detach().cpu())
        return pred_mask_pil
