...
import argparse
import torch
from OCR_Japanese.nets.detectionnet import get_detectionnet
from OCR_Japanese.nets.classifiernet import get_classifiernet
from OCR_Japanese.misc.nihongo import nihongo_class, filter_word
from OCR_Japanese.misc.detection import Detector
from OCR_Japanese.misc.nms import non_max_suppression, column_wordlines
...
def predict_from_image(images):
    ocr_result = my_get_ocr(images, dpi=72)
    predict_result=[]
    for r in ocr_result:
        res=""
        cnt=0
...
def my_get_ocr(filelist,dpi,use_cuda=True,output_detect_img=False,low_gpu_memory=False):
    det_model = 'OCR_Japanese/models/detectionnet.model'
    cls_model = 'OCR_Japanese/models/classifiernet.model'
    if not (os.path.isfile(det_model) and os.path.isfile(cls_model)):
        print('Model file not found.')
        return
    model = get_detectionnet()
    ...
    torch.cuda.empty_cache()
    results = []
    for im, dtct, bbox in zip(filelist, detections, boundings):
        detected_dpi, _, gray_img, scale_image, _ = dtct
        detect_file = {'detected_dpi':detected_dpi,'blocks':[]}
        if len(bbox) > 0:
            for i in range(max([b.sentenceindex for b in bbox])+1):
                bbox_sent = [b for b in bbox if b.sentenceindex == i]
                cbox = column_wordlines(bbox_sent)
